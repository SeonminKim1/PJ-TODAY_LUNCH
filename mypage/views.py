...
def mypage_view(request, year, month):
    if request.method == 'GET':
        ...
        for day, is_day, id, date, weekday, is_diary, score, restaurant_id \
                in zip(date_list, is_date_list, diary_id, diary_date_list, diary_weekday_list, is_diary_list,
                       diary_score, diary_restaurant_id):
        ...
        # 가게 이름 list
        resturant_name_list = list(
            map(lambda x: x.restaurant_name, Restaurant.objects.filter().only('restaurant_name')))
...
def update_diary(request):
    if request.method == 'PATCH':
        ...
        if before_restaurant_id == after_restaurant_id:
            # avg_score만 update
            update_avg_score = ((before_star_score * before_star_count) - before_diary_score + after_diary_score) / before_star_count
